#!/system/bin/sh
# stats_health_summary.sh — HNC hotfix21.6 stats health summary
...
for s in "$DIAG_STATUS" "$IDENT_STATUS" "$RET_STATUS" "$SHADOW_STATUS" "$COMPARE_STATUS"; do
  case "$s" in
...
printf '{"ok":true,"status":"%s","recommendation":"%s","helpers":{"stats_diag":%s,"stats_identity_diag":%s,"stats_retention_diag":%s,"stats_shadow_diag":%s,"stats_compare":%s},"components":{"stats_diag":"%s","stats_identity":"%s","stats_retention":"%s","stats_shadow":"%s","stats_compare":"%s"},"paths":{"json":"%s","text":"%s"}}\n' "$EO" "$ER" "$HAS_DIAG" "$HAS_ID" "$HAS_RET" "$HAS_SHADOW" "$HAS_COMPARE" "$ED" "$EI" "$ET" "$ES" "$EC" "$EJ" "$EX" > "$OUT_JSON"
