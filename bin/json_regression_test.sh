#!/system/bin/sh
# HNC hotfix18.8 JSON writer regression smoke test.
...
TMP="${TMPDIR:-/tmp}/hnc-json-regression-$$"
...
check_json(){
  ...
  # fallback minimal check: file must begin with object/array and have balanced braces outside strings.
  awk '
...
run_json_set(){
  ...
  sh "$JSON_SET" "$@" >/tmp/hnc_json_regression.out 2>&1
  rc=$?
  if [ $rc -ne 0 ]; then
    fail "$name command failed rc=$rc: sh json_set.sh $*"
    cat /tmp/hnc_json_regression.out
    return 1
  fi
  return 0
}
...
# These commands may not exist on older trees. If unavailable, they should fail clearly; keep them as warnings, not hard failures.
if sh "$JSON_SET" name_set aa:bb:cc:dd:ee:ff "$NAME" >/tmp/hnc_json_regression.out 2>&1; then
  check_json "$TMP/data/device_names.json" && ok "device_names.json valid after name_set" || fail "device_names invalid after name_set"
else
  say "[WARN] name_set not supported or failed in this tree; output:"; cat /tmp/hnc_json_regression.out
fi

if sh "$JSON_SET" tpl_set "$TPL" down_mbps 1 >/tmp/hnc_json_regression.out 2>&1; then
  check_json "$TMP/data/templates.json" && ok "templates.json valid after tpl_set" || fail "templates invalid after tpl_set"
else
  say "[WARN] tpl_set not supported or failed in this tree; output:"; cat /tmp/hnc_json_regression.out
fi

rm -rf "$TMP" /tmp/hnc_json_regression.out
[ "$FAIL" -eq 0 ] || exit 1
exit 0
